...
class Fooman_EmailAttachments_Model_Order_Invoice extends Mage_Sales_Model_Order_Invoice
{
    /*
     * need to include the next two function for compatibility with Fooman_Surcharge
     */

    public function getFoomanSurchargeAmount() {
        return $this->getOrder()->getFoomanSurchargeAmount();
    }

    public function getBaseFoomanSurchargeAmount() {
        return $this->getOrder()->getBaseFoomanSurchargeAmount();
    }
    ...
    public function sendEmail($notifyCustomer=true, $comment='')
    {
        ...
        if(Mage::getStoreConfig('sales_email/invoice/attachpdf',$this->getStoreId())){
            ...
            $mailTemplate = Mage::helper('emailattachments')->addAttachment($pdf,$mailTemplate,Mage::helper('sales')->__('Invoice')."_".$this->getIncrementId());
        }
